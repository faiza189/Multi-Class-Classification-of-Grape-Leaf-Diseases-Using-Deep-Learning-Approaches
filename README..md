...
|ResNet50|ImageNet weights frozen with `resnet50.preprocess\_input`, GAP, dense 256 with L2 1e-4, dropout 0.3|about 23.9 M|about 0.5 M|
...
1. Open `Topic\_04\_Model V2.ipynb` in Colab and set Runtime, Change runtime type, GPU.
...
3. Run the remaining cells in order. Do not skip the constants cell or the generator cell, since every later section depends on `IMG\_SIZE`, `train\_generator` and `val\_generator`.
...
|`requirementss.txt`|Python package versions for a local run|
|`FPR\_Draft.docx`|Latest final project report draft|
|`FPR\_Draft\_Chapters1-4.docx`|Earlier report draft, chapters 1 to 4|
|`Grape\_Leaf\_Disease\_Presentation.pptx`|Project presentation|
|`Project Form topic 01 updatd.docx`|Project registration form|
|`skeletal\_ppt\_template.pptx`|University presentation template|
|`nb\_imgs/`|Exported figures from the notebook, with compressed copies in `nb\_imgs/opt/`|

## Reproducibility notes

* `random\_state=42` for pandas sampling in the exploration section and `seed=42` on the generators, so the split is stable across runs.
